...
let red = ref (fun s -> Printf.sprintf "\027[31;1m%s\027[0m" s)

let err msg =
  Format.fprintf !err_fmt "@[<hov>%s: " (!red "Error");
  msg !err_fmt;
...
let error loc msg =
  ...
  let length = e - b in
  Format.fprintf !err_fmt "%s%s" (String.make (b + String.length pref) ' ')
    (!red @@ String.make length '^');
  err msg
