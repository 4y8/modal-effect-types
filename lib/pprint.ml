...
let rec ty_fora ctx fmt = function
  | TForA (k, a) ->
    ...
    fprintf fmt "∀ %s : %a .@ @[%a@]" (Bindlib.name_of v) kind k (ty_fora ctx) a
...
let rec expr_let ctx fmt = function
  | LetMod (mu, nu, m, n) ->
    ...
    fprintf fmt "let%a mod%a %s =@ %a in@ %a"
      (modality ctx) mu (modality ctx) nu (Bindlib.name_of v) (expr_let ctx) m
      (expr_let ctx') n
  | Lam (a, m) ->
    let v, m, ctx' = Bindlib.unbind_in ctx m in
    fprintf fmt "λ%s : %a.%a" (Bindlib.name_of v) (ty_arrow ctx) a
      (expr_let ctx') m
  | TLam (k, m) ->
    let v, m, ctx = Bindlib.unbind_in ctx m in
    fprintf fmt "λ%s : %a.%a" (Bindlib.name_of v) kind k (expr_let ctx) m
  | m -> expr_app ctx fmt m
and expr_app ctx fmt = function
  | TApp (m, a) -> fprintf fmt "%a@ @[%a@]" (expr_app ctx) m (ty_atom ctx) a
  | App (m, n) -> fprintf fmt "%a@ @[%a@]" (expr_app ctx) m (expr_atom ctx) n
